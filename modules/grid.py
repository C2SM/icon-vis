...
def combine_grid_information(ds, grid):
    ...
    if time_coord != 'time':
        ds=ds.rename({'time': ds.coords['time'].attrs['standard_name'], time_coord: 'time'}).expand_dims('time')
...
def get_cell_dim_name(ds,grid):
    ...
    for dim in dims:
        ...
        if dim_value == grid.dims['cell']: # maybe this needs to be dynamic, if grid has ncells as cell dim name
            cell_dim=dim
    return cell_dim
...
def get_edge_dim_name(ds,grid):
    ...
    for dim in dims:
        dim_value=ds.dims[dim]
        if dim_value == grid.dims['edge']: # maybe this needs to be dynamic, for example if grid has ncells as cell dim name, or edges vs edge
            edge_dim=dim
    return edge_dim
...
def add_cell_data(ds, grid):
    ds=ds.assign_coords(clon=('cell', np.float32(grid.coords['clon'].values))
        ).assign_coords(clat=('cell', np.float32(grid.coords['clat'].values))
        ).assign_coords(clat_bnds=(('cell','vertices'), np.float32(grid.coords['clat_vertices'].values))
        ).assign_coords(clon_bnds=(('cell','vertices'), np.float32(grid.coords['clon_vertices'].values)))
...
def add_edge_data(ds, grid):
    ds=ds.assign_coords(elon=('edge', np.float32(grid.coords['elon'].values))
        ).assign_coords(elat=('edge', np.float32(grid.coords['elat'].values))
        ).assign_coords(elat_bnds=(('edge','no'), np.float32(grid.coords['elat_vertices'].values))
        ).assign_coords(elon_bnds=(('edge','no'), np.float32(grid.coords['elon_vertices'].values))
        ).assign_coords(zonal_normal_primal_edge=grid["zonal_normal_primal_edge"]
        ).assign_coords(meridional_normal_primal_edge=grid["meridional_normal_primal_edge"]
    ...
    ds.coords["elat_bnds"][:,1] = xr.apply_ufunc(clat_ind_access, grid["adjacent_cell_of_edge"][1,:])
    ds.coords["elat_bnds"][:,3] = xr.apply_ufunc(clat_ind_access, grid["adjacent_cell_of_edge"][0,:])
    ds.coords["elon_bnds"][:,2] = ds.coords["elon_bnds"][:,1]
    ds.coords["elon_bnds"][:,1] = xr.apply_ufunc(clon_ind_access, grid["adjacent_cell_of_edge"][1,:])
    ds.coords["elon_bnds"][:,3] = xr.apply_ufunc(clon_ind_access, grid["adjacent_cell_of_edge"][0,:])

    normal_edge = xr.concat([ds.zonal_normal_primal_edge, ds.meridional_normal_primal_edge], dim="cart")
    normal_edge = normal_edge / np.linalg.norm(normal_edge, axis=0)
    ds = ds.assign_coords(zn = ds.zonal_normal_primal_edge / np.sqrt(ds.zonal_normal_primal_edge**2 + ds.meridional_normal_primal_edge**2))
    ds = ds.assign_coords(mn = ds.meridional_normal_primal_edge / np.sqrt(ds.zonal_normal_primal_edge**2 + ds.meridional_normal_primal_edge**2))
